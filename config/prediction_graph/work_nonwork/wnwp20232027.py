...
class ForecastPlotter:
    ...
    def fit_models(self): #prophet예측
        m_work = Prophet(changepoint_prior_scale=0.1,seasonality_mode='multiplicative', yearly_seasonality=5)
        m_nonwork = Prophet(changepoint_prior_scale=0.1,seasonality_mode='multiplicative', yearly_seasonality=5)
    ...
    def plot(self): #bokeh 시각화
        ...
        p.xaxis.ticker = FixedTicker(ticks=(pd.date_range(start='2013', end='2031', freq='YS').astype(int64) / 10**6))
...
if __name__ == "__main__":  
    ...
    forecast_plotter = ForecastPlotter(data, ['work_demo', 'nonwork_demo'], '2023-2027 생산가능인구/생산불가능인구 변화')
