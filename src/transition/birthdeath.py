...
class birthdeath:
    def __init__(self):
        self.config = '.\\config\\birth_death\\under2022\\'
        pass
        #sys.stderr.write('__init__: '+os.getcwd()+ self.config +'2013.csv') #확인용

    def showdata(self):
        # csv모듈의 reader함수를 이용해 csv파일을 읽어들여 각 행을 구분
        #path = os.path.join(cwd, './../config/birth_death/under2022/')
        abspath = os.path.join(os.getcwd() + self.config)
        with open(  abspath +'2013.csv', encoding='utf-8') as f:
            reader = csv.reader(f)
            birth_death_data = []
            age_data = []
            for row in reader:
    ...
    def get_data(self, year):
        try:
            with open( self.config  + str(year) + '.csv', encoding='utf-8') as f:
                reader = csv.reader(f)
                birth_death_data = []
                age_data = []
                for row in reader:
            ...
            sys.stderr.write("get_data: ", FileNotFoundError)
