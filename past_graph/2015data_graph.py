...
with open('../tool/birth&death_data/2015data.csv') as f:        # csv모듈의 reader함수를 이용해 csv파일을 읽어들여 각 행을 구분
    reader = csv.reader(f)
    birth_death_data = []
    age_data = []
    for row in reader:
...
p1 = figure(y_range=birth_death_df['type'], title=Title(text="2015년 출생아 수 사망자 수", align="center", text_font_size="22px", text_font="Consolas", text_font_style="bold"), height=500, width=500)
